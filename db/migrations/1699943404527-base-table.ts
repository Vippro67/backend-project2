...
export class BaseTable1699943404527 implements MigrationInterface {
    name = 'BaseTable1699943404527'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`group\` (\`id\` varchar(36) NOT NULL, \`name\` varchar(255) NOT NULL, \`avatar\` varchar(255) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`message\` (\`id\` varchar(36) NOT NULL, \`content\` varchar(255) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`sender_id\` varchar(36) NOT NULL, \`receiver_id\` varchar(36) NULL, \`group_id\` varchar(36) NULL, \`media_id\` varchar(36) NULL, UNIQUE INDEX \`REL_7d3846d3e3d5fd07241e1aebff\` (\`media_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`tag\` (\`id\` varchar(36) NOT NULL, \`name\` varchar(255) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`relationship\` (\`user_id\` varchar(255) NOT NULL, \`friend_id\` varchar(255) NOT NULL, \`isFriend\` tinyint NOT NULL DEFAULT 0, \`isBlocked\` tinyint NOT NULL DEFAULT 0, \`status\` enum ('pending', 'confirmed') NOT NULL DEFAULT 'pending', \`isFollowing\` tinyint NOT NULL DEFAULT 0, PRIMARY KEY (\`user_id\`, \`friend_id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`user\` (\`id\` varchar(36) NOT NULL, \`first_name\` varchar(255) NOT NULL, \`last_name\` varchar(255) NOT NULL, \`email\` varchar(255) NOT NULL, \`password\` varchar(255) NOT NULL, \`refresh_token\` varchar(255) NULL, \`avatar\` varchar(255) NULL, \`status\` int NOT NULL DEFAULT '1', \`userType\` varchar(255) NOT NULL DEFAULT 'regular', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`post\` (\`id\` varchar(36) NOT NULL, \`title\` varchar(255) NOT NULL, \`description\` varchar(255) NOT NULL, \`totalLikes\` int NOT NULL DEFAULT '0', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`user_id\` varchar(36) NOT NULL, \`media_id\` varchar(36) NULL, UNIQUE INDEX \`REL_049edb1ce7ab3d2a98009b171d\` (\`media_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`comment\` (\`id\` varchar(36) NOT NULL, \`comment\` varchar(255) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`post_id\` varchar(36) NOT NULL, \`user_id\` varchar(36) NOT NULL, \`media_id\` varchar(36) NULL, \`replied_comment_id\` varchar(36) NULL, UNIQUE INDEX \`REL_64555a834f96e9d38fd8a47908\` (\`media_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`user_groups\` (\`group_id\` varchar(36) NOT NULL, \`user_id\` varchar(36) NOT NULL, INDEX \`IDX_4c5f2c23c34f3921fbad2cd394\` (\`group_id\`), INDEX \`IDX_95bf94c61795df25a515435010\` (\`user_id\`), PRIMARY KEY (\`group_id\`, \`user_id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`post_likes\` (\`user_id\` varchar(36) NOT NULL, \`post_id\` varchar(36) NOT NULL, INDEX \`IDX_9b9a7fc5eeff133cf71b8e06a7\` (\`user_id\`), INDEX \`IDX_b40d37469c501092203d285af8\` (\`post_id\`), PRIMARY KEY (\`user_id\`, \`post_id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`relationship\` ADD CONSTRAINT \`FK_ce5052f97510956b523063fc499\` FOREIGN KEY (\`user_id\`) REFERENCES \`user\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`relationship\` ADD CONSTRAINT \`FK_2c5ece7575fa0a6b33b9a4923ec\` FOREIGN KEY (\`friend_id\`) REFERENCES \`user\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`relationship\` DROP FOREIGN KEY \`FK_2c5ece7575fa0a6b33b9a4923ec\``);
        await queryRunner.query(`ALTER TABLE \`relationship\` DROP FOREIGN KEY \`FK_ce5052f97510956b523063fc499\``);
