...
export class BaseTable1703181752131 implements MigrationInterface {
    name = 'BaseTable1703181752131'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`relationship\` (\`id\` varchar(36) NOT NULL, \`isFriend\` tinyint NOT NULL DEFAULT 0, \`isBlocked\` tinyint NOT NULL DEFAULT 0, \`status\` enum ('pending', 'confirmed') NOT NULL DEFAULT 'pending', \`isFollowing\` tinyint NOT NULL DEFAULT 0, \`userId\` varchar(36) NULL, \`friendId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`user\` (\`id\` varchar(36) NOT NULL, \`first_name\` varchar(255) NOT NULL, \`last_name\` varchar(255) NOT NULL, \`email\` varchar(255) NOT NULL, \`password\` varchar(255) NOT NULL, \`refresh_token\` varchar(255) NULL, \`avatar\` varchar(255) NULL, \`status\` int NOT NULL DEFAULT '1', \`userType\` enum ('regular', 'admin') NOT NULL DEFAULT 'regular', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`post\` (\`id\` varchar(36) NOT NULL, \`title\` varchar(255) NOT NULL, \`description\` varchar(255) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`user_id\` varchar(36) NOT NULL, \`media_id\` varchar(36) NULL, \`group_id\` varchar(36) NULL, UNIQUE INDEX \`REL_049edb1ce7ab3d2a98009b171d\` (\`media_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`media\` (\`id\` varchar(36) NOT NULL, \`link\` varchar(255) NOT NULL, \`type\` enum ('image', 'video') NOT NULL, \`post_id\` varchar(36) NULL, \`comment_id\` varchar(36) NULL, \`message_id\` varchar(36) NULL, UNIQUE INDEX \`REL_20dc152fa567df67110b94d6f1\` (\`post_id\`), UNIQUE INDEX \`REL_06df0ecbcc0ce3e024007aef72\` (\`comment_id\`), UNIQUE INDEX \`REL_a33f77c90759d1f2c799941e7d\` (\`message_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`pet\` (\`id\` varchar(36) NOT NULL, \`avatar\` varchar(255) NULL, \`name\` varchar(255) NOT NULL, \`species\` varchar(255) NOT NULL, \`sex\` varchar(255) NOT NULL, \`breed\` varchar(255) NOT NULL, \`date_of_birth\` datetime NOT NULL, \`description\` varchar(255) NOT NULL, \`owner_id\` varchar(36) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
